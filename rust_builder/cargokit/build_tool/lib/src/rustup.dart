...
class Rustup {
  ...
  String resolveToolchain(String toolchain) => _bestMatch(toolchain)?.name ?? toolchain;
  ...
  String get _hostTriple {
    final cached = _defaultHost;
    if (cached != null) {
      return cached;
    }
    final res = runCommand('rustup', ['show']);
    final line = res.stdout.toString().split('\n').firstWhere(
          (entry) => entry.startsWith('Default host:'),
          orElse: () => '',
        );
    _defaultHost = line.replaceFirst('Default host:', '').trim();
    return _defaultHost!;
  }

  _Toolchain? _bestMatch(String toolchain) {
    final matches = _installedToolchains.where(
      (candidate) => _matchesRequestedToolchain(
        installedName: candidate.name,
    ...
    final host = _hostTriple;
    for (final candidate in matches) {
      if (host.isNotEmpty && candidate.name.endsWith('-$host')) {
        return candidate;
      }
    }
  ...
  List<String>? _installedTargets(String toolchain) => _bestMatch(toolchain)?.targets;
